...
(function($){
	...
	$( document ).ready( function(){
		...
		$( '.toggle-switch' ).on( 'click', 'span', function() {
					
			var $con = $(this).closest( '.toggle-switch' ),
				value = $(this).data( 'value' );

			$(this).addClass('active').siblings().removeClass( 'in-active active' );
			
			if( value ) {
				$con.find( 'input' ).prop( 'checked', true );
			}
			else {
		...
		$( '.repeatable-fields' ).on( 'click', '.add-repeatable-field', function( e ) {
			...
			$template.find('.field-group .field').each( function( index, el ) {
				...
				if( 'text' == type || 'color' == type ) {
					$el.find( 'input' ).attr( 'name', baseKey + '['+key+']' + '['+fieldName+']' );
				}
				else if( 'textarea' == type ) {
					$el.find( 'textarea' ).attr( 'name', baseKey + '['+key+']' + '['+fieldName+']' );
				}
				else if( 'radio' == type ) {
					$el.find( 'radio' ).attr( 'name', baseKey + '['+key+']' + '['+fieldName+']' );
				}
