...
if( ! class_exists( 'octagon_kc_elements_portfolio_extend_slider_module' ) ) {

	class octagon_kc_elements_portfolio_extend_slider_module {
		...
		public function __construct(){
			...
			add_shortcode( 'octagon_kc_elements_portfolio_extend_slider', array( $this, 'content' ) );
		}
		...
		public function map() {
			...
			kc_add_map(
				array(
					'octagon_kc_elements_portfolio_extend_slider' => array(
						'name' => esc_html_x( 'Portfolio Extend Slider', 'shortcode-map', 'octagon-kc-elements' ),
						'icon' => 'icon-portfolio',
		...
		public function get_slider_data( $atts ) {
			...
			$slide_data = apply_filters( 'octagon_portfolio_extend_slider_data', $slide_data );
		...
		public function content( $atts ) {
			extract( shortcode_atts( array(
				'style'            => 'content-overlap', // content-overlap
				'title_tag'        => 'h3', // h1, h2, h3, h4, h5, h6, p
				'show_like'        => 'show', // show, hide
				'show_category'    => 'show', // show, hide
				'pagination'       => 'loadmore', // loadmore, infinite-scroll, number, next-prev
				'ex_class'         => '', // show, hide
			...
			$output = '<div class="'. esc_attr( implode( ' ', $wrapper_class ) ) .'">';
				$output .= octagon_kc_elements_get_shortcode_template( 'portfolio-extend-slider-'. $style, $atts );
			$output .= '</div>';
...
new octagon_kc_elements_portfolio_extend_slider_module;